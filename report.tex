...
AI compute is becoming an infrastructure-execution business. The constraint set is shifting from accelerator availability alone to facility power, grid interconnection, construction execution, financing capacity, and counterparty credit.

\medskip
At Q2 2026, we estimate \textbf{7.8~GW} of operational AI-relevant facility power across the Western-aligned frontier ecosystem. Announced commitments imply a \textbf{51.4~GW} raw 2030 facility-power horizon, equivalent to \textbf{43.9~GW} of IT load under our row-level PUE bridge. Because most of the horizon remains forward commitment rather than energized capacity, our realization model places the 2030 median at \textbf{32.5~GW facility}, with a p10--p90 range of \textbf{24.6--38.4~GW}.

\medskip
The build-out carries a \textbf{\$1.2--1.5T} capex envelope and is underwritten by roughly \textbf{\$550B} of RPO and take-or-pay commitments. The resulting system is concentrated across a small set of hyperscalers, labs, neoclouds, and sovereign sponsors. The main risks are grid delay, financing stress, chip-roadmap slippage, and lab-revenue/RPO enforceability.
};
\end{tikzpicture}

\vfill
{\color{softGrey}\small
Data cutoff: April 24, 2026\quad\textbullet\quad Publication: April 24, 2026\\[2pt]
Primary data: Epoch AI, \emph{Frontier Data Centers} (snapshot 2026-04-24, incorporating the 2026-04-22 changelog revision to Stargate Abilene and the 2026-04-10 revision to Meta Prometheus).\footnote{Epoch AI, \emph{Frontier Data Centers} dataset and changelog, retrieved April 24, 2026. \url{https://epoch.ai/data/data-centers}. Methodology (\url{https://epoch.ai/data/data-centers-documentation}) documents that the dataset's \texttt{Power (MW)} field is total facility power (IT accelerators plus cooling, lighting, and conversion losses); cooling-model power estimates can in principle vary by up to a factor of 2 against ground truth, though Epoch's validation against known-capacity sites shows agreement within approximately 50\%; capital-cost estimates apply a uniform \$44~B/GW (\$30~B hardware + \$14~B infrastructure) and do not vary by site. These bands are consistent with our T2 realization-probability range of 80--95\% and the T3 range of 65--90\% and provide external validation for the six-tier prior framework.} Supplementary disclosures cited in footnotes throughout.
}
\end{center}
...
\textcolor{softGrey}{5.} & Capital requirements and financing structure & 11 \\
...
Equity (public markets + private) & 150 & 160 & Founder dilution; VC LPs \\
\midrule
\textbf{Total capital envelope} & \textbf{1{,}200} & \textbf{1{,}560} & \\
\bottomrule
\end{tabular}
...
The \$1.2--1.5T capital envelope has two equally valid aggregate views. The capex bridge shows \emph{what the money physically buys}; the financing bridge shows \emph{whose balance sheet the money flows off}. They must balance in aggregate but answer different accounting questions. The \$550B of contracted remaining performance obligations (RPO) is \emph{neither}: it is the revenue underwriting that \emph{enables} the debt-financing channel. Oracle's \$300B from OpenAI, Anthropic's \$100B+ from Amazon, Nebius's \$27B from Meta, and related lab commitments are future revenue obligations, not capex channels.
...
\addplot[fill=chartA,draw=white] coordinates {(750,capex)};
\addlegendentry{Accelerators $\sim$\$750B}
\addplot[fill=chartB,draw=white] coordinates {(205,capex)};
\addlegendentry{Shell + fit-out $\sim$\$205B}
\addplot[fill=chartC,draw=white] coordinates {(230,capex)};
\addlegendentry{Power, cooling, substation $\sim$\$230B}
\addplot[fill=chartD,draw=white] coordinates {(115,capex)};
\addlegendentry{Networking + interconnect $\sim$\$115B}
\addplot[fill=chartE,draw=white] coordinates {(80,capex)};
\addlegendentry{Land, permitting, transmission $\sim$\$80B}
\addplot[draw=none,fill=none,forget plot,nodes near coords={}] coordinates {(0,dummy)};
\end{axis}
\end{tikzpicture}
\caption{Capex bridge for the Western 2026--2030 \$1.2--1.5T envelope. Accelerator silicon is the dominant share (approximately 54\% of the midpoint) because per-chip ASPs in the B300/Rubin/Trainium-3 era exceed \$30--50k per unit and cluster densities have risen to 1{,}000+~H100e/MW. Physical shell, power, and cooling together are approximately 32\% of the midpoint; networking is approximately 8\%; land, permitting, and transmission are approximately 6\%. Ranges reflect vendor-specific ASP uncertainty and the silicon-uplift sensitivity in \S\ref{subsec:h100e-sensitivity}.\protect\footnotemark}
\end{figure}
\footnotetext{Author's aggregation. Accelerator costs are extrapolated to the 51.4~GW facility horizon at a blended \$17--20~M per facility-MW. Physical shell, power, and cooling use a \$44~B/GW Epoch infrastructure benchmark for non-Epoch commitments and a \$30~B/GW Epoch benchmark for the physically-evidenced floor. Networking, land, permitting, and transmission are estimated at \$1.5--3~M/MW from the row-level infrastructure audit.}
...
\addplot[fill=chartA,draw=white] coordinates {(600,financing)};
\addlegendentry{Hyperscaler operating cash $\sim$\$600B}
\addplot[fill=chartB,draw=white] coordinates {(240,financing)};
\addlegendentry{Corporate debt $\sim$\$240B}
\addplot[fill=chartC,draw=white] coordinates {(185,financing)};
\addlegendentry{SPV / project-finance debt $\sim$\$185B}
\addplot[fill=chartD,draw=white] coordinates {(200,financing)};
\addlegendentry{Sovereign capital $\sim$\$200B}
\addplot[fill=chartE,draw=white] coordinates {(155,financing)};
\addlegendentry{Equity $\sim$\$155B}
\addplot[draw=none,fill=none,forget plot,nodes near coords={}] coordinates {(0,dummy)};
\end{axis}
\end{tikzpicture}
\caption{Financing bridge for the same \$1.2--1.5T capital envelope, decomposed by source of funds. Hyperscaler operating cash flow is the dominant channel (approximately 43\% of the midpoint) because Microsoft, Amazon, Google, and Meta collectively generate roughly \$400B of annual free cash flow before capex. Corporate and SPV debt together add approximately 31\%; the SPV channel is structured against 15-year take-or-pay RPO, wrapped to investment-grade at the SPV level, and issued into private-credit markets. Sovereign capital from MGX, HUMAIN, UK DSIT, and Reliance contributes approximately 14\%. Equity absorbs the remaining 11\%. The \$550B RPO does \emph{not} appear as a stand-alone channel here; it is the revenue underwriting that enables the SPV/project-finance debt channel.\protect\footnotemark}
\end{figure}
\footnotetext{Author's aggregation. Hyperscaler channel: Big-Four CY2025 actual plus CY2026 guidance times AI share, extended 2027--2030 at roughly flat dollar levels. Corporate debt: hyperscaler balance-sheet additions plus CoreWeave, Nebius, Applied Digital, Crusoe corporate issuance. SPV / project-finance: Applied Digital--CoreWeave SPV, CoreWeave Blackstone facility, Nebius ABS, and Crusoe ABS, all wrapped to investment-grade via lab-revenue RPO. Sovereign: MGX \$100--200B per OpenAI Stargate announcement + HUMAIN \$10B AMD JV + UK DSIT £5B + RIL estimated \$30--50B Jamnagar phase-one. Equity: OpenAI secondary rounds, Anthropic Series F/G, CoreWeave public-float, Nebius AMS float.}
...
\newpage
\subsection{Monte Carlo realization distribution}
...
The model's downside is concentrated in grid delay, neocloud financing stress, and lab-revenue/RPO enforceability. Its upside is concentrated in faster interconnection, continued financing access, T4 over-realization, and silicon-density gains. The raw announced horizon is therefore best read as an upper accounting boundary; the Monte Carlo median is the base realization case.
