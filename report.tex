...
As of 2026-04-28, the six-tier framework places approximately \textbf{6.52~GW of
directly-disclosed operational AI compute} at Tier~1 (tier-clean; corresponding to
approximately \textbf{6.77~GW at facility power} including T6 analyst-inferred operational capacity
from Voltage~Park / Lightning and the xAI Colossus~2 residual). Announced commitments through 2030 imply a \textbf{raw horizon of approximately 47.9~GW facility
power} across Western-aligned operators --- equivalent to approximately \textbf{40.9~GW
on the IT-load bridge}. Under our realization-probability framework (T1 at 100\% through
T6 at 25\%) the deterministic tier-weighted point estimate is approximately \textbf{32.6~GW
...
T5 stretch targets) is approximately \textbf{39.0~GW facility}. The \textbf{conservative
case} raw (Tiers~1+2+3 only) is approximately \textbf{22.9~GW facility}; the \textbf{row-uncertainty
high envelope} is approximately \textbf{54.2~GW facility}. The associated capital envelope is
approximately \textbf{\$1.77~trillion (range \$1.44--2.25~trillion)}, separately underwritten by approximately
...
A separate approximately \textbf{4.96~GW facility} sovereign stretch annex pipeline in the UAE,
...
In Q2 2026, the Western-aligned tier-clean operational AI compute fleet totals approximately \textbf{6.52~GW} facility power (T1), with a further approximately \textbf{0.25~GW facility} T6-inferred from neocloud sources (Voltage Park / Lightning and the xAI Colossus~2 residual). The chip footprint is approximately \textbf{4.2~million H100-equivalent}.\footnote{Epoch AI, \emph{Frontier Data Centers} dataset, snapshot April 24, 2026, incorporating the 2026-04-22 changelog entry revising Stargate Abilene from a 600~MW April-1 milestone to ~200~MW actual operational. Epoch's ``Power (MW)'' field reports total facility power (GPUs + cooling + lighting + electrical losses); see \url{https://epoch.ai/data/data-centers-documentation}. H100-equivalent (H100e) normalizes GPU throughput by 8-bit operations per second relative to the NVIDIA H100 baseline.} Announced commitments through 2030 across the same operators, including hyperscalers, frontier labs, and the independent neoclouds, imply a raw \textbf{47.9~GW facility} horizon across the Western-aligned operator set (\textbf{40.9~GW} IT-load bridge), with a further approximately \textbf{4.96~GW facility} in the sovereign stretch annex (Rev-4.3 added Reliance Andhra 1.0~GW T5 atom; Digital Connexion Vizag deduped as same-program). Under tier-default probability weighting, the expected 2030 realization is approximately \textbf{32.6~GW facility}; the T1+T2+T3 conservative case is approximately \textbf{22.9~GW facility}; and the row-uncertainty high envelope is approximately \textbf{54.2~GW facility}.

\textbf{The central fact} is that approximately 85\% of the 47.9~GW facility raw 2030 horizon is not yet operational. On a probability-weighted basis, the expected realization of approximately 32.6~GW facility leaves approximately 78\% still to build, and the Monte Carlo median of approximately 29.0~GW facility (\S\ref{subsec:monte-carlo}) implies that even in the base realization case approximately 75\% of the 2030 horizon remains to be built. The AI compute stack of the late 2020s is overwhelmingly a forward commitment, not an in-place fleet.
...
\textbf{Where this section ends and the next begins.} Section~\ref{sec:scale} states the headline aggregates --- 6.52~GW tier-clean operational capacity, 47.9~GW raw 2030 horizon, 29.0~GW Monte~Carlo median, approximately \$1.77~trillion capital envelope, 78.0~million H100-equivalents. These are scale numbers, multiplications. Before scaling, the multiplicand. The next section decomposes one AI facility gigawatt into its physical bill of materials so every aggregate downstream resolves to a unit that has been priced, assembled, and reconciled to the external literature. We then return in Section~\ref{sec:scale-body} to the power-unit conventions, the compute-versus-physical-capacity bridge, and the operator concentration on which the headline numbers rest.
...
Deterministic tier-weighted point & 32.6 & \$1.04--1.63T \\
Monte~Carlo p90 & 34.8 & \$1.12--1.75T \\
Raw non-stretch horizon (announced minus T5) & 39.0 & \$1.25--1.95T \\
...
\bottomrule
\end{tabular}
\caption{IT-load-equivalent capacity mapped from the 47.9~GW facility-basis Western 2030 horizon under five PUE assumptions. The central IT-load bridge is 40.9~GW under row-level PUE assumptions. Comparisons against utility-contracted-MW or grid-interconnect datasets should use the facility figures; comparisons against chip-nameplate aggregates should use the IT-load equivalents.}
\end{table}
...
\textbf{Approximately 85\% of the 47.9~GW facility raw horizon is not yet operational; probability-weighted expected realization by 2030 is approximately 32.6~GW facility, of which roughly 78\% is not yet operational today.} The build-out is therefore a forward execution bet, not an in-place compute base. The principal risks are concentrated in four categories: grid interconnection, chip delivery, counterparty concentration, and the realization of outer-year stretch targets (particularly Meta Hyperion, Reliance Jamnagar, and the Anthropic--Amazon full 5~GW). Each is priced into the scenario matrix in \S\ref{subsec:stress-matrix}.

\subsection{Confidence decomposition of the 2030 horizon}

Of the 47.9~GW facility Western horizon, the six-tier evidence split is: \textbf{T1 Operational} 14\% (6.52~GW, 100\% realization), \textbf{T2 Under construction / physically evidenced} 25\% (11.91~GW, 88\% default), \textbf{T3 Firm commercial} 9\% (4.50~GW, 78\% default), \textbf{T4 Announced site-level plan} 33\% (15.79~GW, 58\% default), \textbf{T5 LOI / stretch target} 18\% (8.86~GW, 32\% default; widened by Rev-4.3 \texttt{anthropic\_google\_broadcom\_physical\_tpu} +2.7~GW), and \textbf{T6 Analyst inference} 1\% (0.33~GW, 25\% default). Probability weighting across the six tiers yields approximately \textbf{32.6~GW facility} as the deterministic expected 2030 realization, materially below the 47.9~GW announced horizon and materially above the 22.9~GW conservative case (T1+T2+T3 only). The Monte Carlo median (\S\ref{subsec:monte-carlo}) at approximately 29.0~GW facility is the base realization case.
...
\end{axis}
\end{tikzpicture}
\caption{Distribution of the 47.9~GW facility Western horizon across six evidence tiers. Per-tier realization probability is the tier-default midpoint with per-row override possible. Probability-weighted sum across the six tiers = approximately \textbf{32.6~GW facility}; the next figure shows the same segments re-scaled by tier-default probability.\protect\footnotemark}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Same six tiers as the preceding figure, with each segment re-scaled by its tier-default realization probability. The expected 2030 Western realization is approximately \textbf{32.6~GW facility}, the right-hand edge of the stacked bar. The Monte Carlo median is approximately 29.0~GW facility after the two-sided scenario engine (\S\ref{subsec:monte-carlo}). The T1+T2+T3 conservative subset (6.52 + 10.48 + 3.51 = 20.51~GW expected, or 22.9~GW announced facility) sits as the conservative reading; the 54.2~GW facility row-uncertainty high envelope is shown separately in the canonical totals. Basis is facility power; see \S\ref{subsec:power-units}.}
\end{figure}

\subsection{Scenario ranges: non-stretch 39.0 GW to row-uncertainty high 54.2 GW (facility)}

Stripping the stretch-target-only tier (T5) and compressing speculative completions yields a \textbf{raw non-stretch reading} of approximately \textbf{39.0~GW facility} (announced minus T5). Realising every stretch on disclosed timeline yields a \textbf{row-uncertainty high envelope} of approximately \textbf{54.2~GW facility}. The deterministic tier-weighted base of \textbf{32.6~GW facility} sits between the two because tier-default probabilities discount each tier. The Monte Carlo median (\S\ref{subsec:monte-carlo}) falls lower still at approximately \textbf{29.0~GW facility}, reflecting per-tier variance, correlated downside states, demand/RPO stress, and upside cases.
...
The 32.6~GW facility tier-weighted horizon is a single point. The realization distribution combines tier-level uncertainty with the scenario architecture above. Each draw samples realization rates by tier, applies the systemic infrastructure state, samples conditional physical-capacity shocks, applies demand/RPO stress, and then applies the independent silicon, mix, and upside cases.
...
\bottomrule
\end{tabular}
\caption{Monte Carlo distribution of the 2030 Western realized horizon (facility basis), 10{,}000 draws. Seed 20260424, tier rollup T1=6.52, T2=11.91, T3=4.50, T4=15.79, T5=8.86, T6=0.33 facility. \textbf{The median (29.0~GW facility) sits below the 32.6~GW facility tier-weighted point} because downside mechanisms remain larger than the modeled upside states. This is a realization distribution, not an accounting horizon. IT-load bridge at blended PUE~1.20: approximately 27.4~GW IT (see \S\ref{subsec:power-units}).}
\end{table}

\textbf{Tail probabilities from the simulation (facility basis):}
\begin{itemize}\itemsep0pt
\item $P(\text{realized 2030 GW} < 22.9$ conservative case$) \approx 24.0\%$
\item $P(\text{realized 2030 GW} < 32.6$ tier-weighted point$) \approx 75.8\%$
\item $P(\text{realized 2030 GW} < 39.0$ raw non-stretch$) \approx 99.9\%$
\item $P(\text{realized 2030 GW} < 47.9$ announced horizon$) = 100\%$
\end{itemize}

We treat the Monte Carlo median of approximately \textbf{29.0~GW facility} as the base realization case, with a p10--p90 interdecile range of \textbf{[21.1, 34.8]~GW facility}. The announced 47.9~GW and raw non-stretch 39.0~GW horizons are accounting totals, not expected realized outcomes. For stress testing, p10 (approximately 21.1~GW facility) is the downside case.
...
\midrule
45\% & 34.1 & 30.5 & 22.4--32.6 \\
58\% base & 32.6 & 29.0 & 21.1--34.8 \\
...
\end{axis}
\end{tikzpicture}
\caption{Sovereign stretch annex pipeline, 2030 target GW. \textbf{Unit:} GW facility power at commissioning; IT-load bridge is reported separately from row-level PUE assumptions. \textbf{Rows:} Stargate UAE (1.40~GW), UAE G42--Khazna (0.26~GW), xAI--HUMAIN Saudi (0.50~GW), AMD--HUMAIN Saudi (0.50~GW), UK Culham AI Growth Zone (0.30~GW), and Reliance Jamnagar (1.00~GW). \textbf{Aggregate:} 4.96~GW facility announced (approximately 3.83~GW IT-load bridge), 2.03~GW facility probability-weighted. \textbf{Not included in Western 47.9~GW facility denominator.}\protect\footnotemark}
\end{figure}
...
Because a single gigawatt of capacity may be referenced across multiple announcements (e.g., Microsoft's Fairwater commitment and OpenAI's Stargate reference; Amazon's Project Rainier and Anthropic's Trainium commitments), capacity was consolidated at the site level. Where an announcement references an existing Epoch-tracked site, the announcement's reported capacity replaces the Epoch figure only if it exceeds the Epoch buildout; otherwise the Epoch figure is retained. Chip procurement commitments (OpenAI--NVIDIA 10~GW, OpenAI--AMD 6~GW, OpenAI--Broadcom 10~GW, Anthropic--Google 4.5~GW TPU) are recorded as chip-vendor commitments rather than physical-site commitments, since the chips deploy into Stargate, Azure, Fluidstack, or Google-owned shells already accounted for at the site level. The resulting unduplicated physical-site census is the 47.9 GW facility Western 2030 horizon referenced throughout this paper; the corresponding IT-load bridge is 40.9 GW under row-level PUE assumptions. (Anthropic-Google/Broadcom is now treated as a T5 physical-overlay row at 2.7 GW central per Rev-4.3, replacing the prior Class B chip-procurement zero treatment that triggered reviewer~\#3.)
