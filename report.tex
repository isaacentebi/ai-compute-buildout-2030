...
\emph{Rev-4.3 changes (2026-04-28, second pass after reviewer triage).} Three reviewer findings that survived Rev-4.2 first-pass triage are now closed: (i)~\textbf{Reviewer~\#5 CoreWeave double-count} ($-0.85$~GW): the FY2025 10-K reports ``43 data centers with over 850~MW of active power'' AND ``total contracted power capacity was approximately 3.1~GW''; the active is a SUBSET of the contracted, not additive; \texttt{coreweave\_operational\_disclosed} retained for tier-table T1 visibility but excluded from totals. (ii)~\textbf{Reviewer~\#3 Anthropic-Google/Broadcom asymmetry}: per user direction (the Anthropic-Google deal should exist in the paper at appropriate tier), a new T5 atom \texttt{anthropic\_google\_broadcom\_physical\_tpu} with central 2.7~GW facility (4.5~GW IT exposure $\times$ 50\% Epoch overlap) and range 0--5.4~GW; Anthropic-Azure central reduced 1.180 $\rightarrow$ 0.590~GW with 50\% Fairwater overlap (Microsoft + NVIDIA confirm Vera Rubin deploys at Fairwater Wisconsin/Atlanta). (iii)~\textbf{Reviewer~\#7 Meta Hyperion source}: local-cache transcript URL replaced with publicly hosted Q2 2025 transcript PDF; source\_support upgraded to primary\_company. Net headline: Western raw facility horizon \textbf{47.9~GW} (range \textbf{39.5--54.2~GW}); deterministic tier-weighted \textbf{32.6~GW}; conservative T1+T2+T3 \textbf{22.9~GW}; capital envelope \textbf{\$1.77~trillion (range \$1.44--2.25~T)}. Sovereign sidebar widens 3.96 $\rightarrow$ 4.96~GW (new \texttt{reliance\_andhra\_1gw\_ai\_dc\_mou} atom, Digital Connexion Vizag deduped as same-program). HUMAIN xAI source date refreshed; HUMAIN AMD primary source corrected.

\emph{Rev-4.2 changes (2026-04-28, first pass).} The database is rebuilt around atom-level provenance and site-level dedupe. Rev-4.2 also adds \texttt{dedupe\_audit.csv}, \texttt{row\_delta\_ledger.csv}, \texttt{docs/tier\_table.md}, and per-contract drilldowns under \texttt{contracts/}. The largest first-pass corrections are Anthropic--AWS (3.800 $\rightarrow$ 1.973~GW), Fairwater International (0.805 $\rightarrow$ 0.300~GW), Nscale--Microsoft (0.900 $\rightarrow$ 0.240~GW), and Crusoe future capacity (1.980 $\rightarrow$ 1.800~GW and T3 $\rightarrow$ T4). Sovereign rows are reported in the sovereign stretch annex and remain outside the Western denominator.
