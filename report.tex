...
At the end of Q2 2026, the six-tier framework places approximately \textbf{7.37~GW of
...
from Voltage~Park / Lightning and the xAI Colossus~2 residual). Announced commitments through 2030 imply a \textbf{raw horizon of approximately 49.8~GW facility
power} across Western-aligned operators --- equivalent to approximately \textbf{42.5~GW
on the IT-load bridge}. Under our realization-probability framework (T1 at 100\% through
T6 at 25\%) the deterministic tier-weighted point estimate is approximately \textbf{35.1~GW
...
31.4~GW facility} with an \textbf{interdecile range of [23.4, 37.2]~GW} --- our
probability-weighted realization base case. The \textbf{raw non-stretch horizon} (announced minus
T5 stretch targets) is approximately \textbf{43.1~GW facility}. The \textbf{conservative
case} raw (Tiers~1+2+3 only) is approximately \textbf{26.3~GW facility}; the \textbf{full-realization
ceiling} is approximately \textbf{53.1~GW facility}. The associated capital envelope is
approximately \textbf{\$1.84~trillion (range \$1.49--2.34~trillion)}, separately underwritten by approximately
...
A separate approximately \textbf{3.96~GW facility} sovereign-AI pipeline in the UAE,
...
In Q2 2026, the Western-aligned tier-clean operational AI compute fleet totals approximately \textbf{7.37~GW} facility power (T1), with a further approximately \textbf{0.25~GW facility} T6-inferred from neocloud sources (Voltage Park / Lightning and the xAI Colossus~2 residual). The chip footprint is approximately \textbf{4.2~million H100-equivalent}.\footnote{Epoch AI, \emph{Frontier Data Centers} dataset, snapshot April 24, 2026, incorporating the 2026-04-22 changelog entry revising Stargate Abilene from a 600~MW April-1 milestone to ~200~MW actual operational. Epoch's ``Power (MW)'' field reports total facility power (GPUs + cooling + lighting + electrical losses); see \url{https://epoch.ai/data/data-centers-documentation}. H100-equivalent (H100e) normalizes GPU throughput by 8-bit operations per second relative to the NVIDIA H100 baseline.} Announced commitments through 2030 across the same operators, including hyperscalers, frontier labs, and the independent neoclouds, imply a raw \textbf{49.8~GW facility} horizon across the Western-aligned operator set (\textbf{42.5~GW} IT-load bridge), with a further approximately \textbf{3.96~GW facility} in the sovereign-AI sidebar. Under tier-default probability weighting, the expected 2030 realization is approximately \textbf{35.1~GW facility}; the T1+T2+T3 conservative case is approximately \textbf{26.3~GW facility}; and the full-realization ceiling is approximately \textbf{53.1~GW facility}.

\textbf{The central fact} is that approximately 85\% of the 49.8~GW facility raw 2030 horizon is not yet operational. On a probability-weighted basis, the expected realization of approximately 35.1~GW facility leaves approximately 78\% still to build, and the Monte Carlo median of approximately 31.4~GW facility (\S\ref{subsec:monte-carlo}) implies that even in the base realization case approximately 75\% of the 2030 horizon remains to be built. The AI compute stack of the late 2020s is overwhelmingly a forward commitment, not an in-place fleet.
...
\addplot[color=accentOchre,line width=1.2pt,mark=square*,mark size=1.7pt,mark options={fill=accentOchre}] coordinates {
  ...
  (2027, 30.8) (2028, 46.8) (2029, 49.8) (2030, 49.8)
};
...
Multiplying one facility GW by the announced horizon resolves the headline numbers. The Western 2026--2030 capital envelope at the raw 49.8~GW announced horizon is approximately \textbf{\$1.84~trillion central, with a defensible range of \$1.49--2.34~trillion} depending on operator mix, BTM-generation share, and silicon-vintage assumptions.
...
\midrule
Monte~Carlo p10 (\S\ref{subsec:monte-carlo}) & 23.4 & \$0.70--1.10T \\
Scenario-adjusted median (Monte~Carlo p50) & 31.4 & \$0.94--1.48T \\
Deterministic tier-weighted point & 35.1 & \$1.05--1.65T \\
Monte~Carlo p90 & 37.2 & \$1.12--1.75T \\
Raw non-stretch horizon (announced minus T5) & 43.1 & \$1.29--2.02T \\
\textbf{Raw announced horizon} & \textbf{49.8} & \textbf{\$1.49--2.34T (central \$1.84T)} \\
Full-realization ceiling & 53.1 & \$1.59--2.50T \\
\bottomrule
\end{tabular}
...
\textbf{PUE sensitivity.} The facility-basis 49.8~GW point estimate maps to \emph{IT-load-equivalent} capacity by dividing by the blended PUE; this is the relevant bridge for comparison against chip-nameplate aggregates (NVIDIA's 10~GW LOI, Broadcom's 10~GW XPU commitment).
...
\bottomrule
\end{tabular}
\caption{IT-load-equivalent capacity mapped from the 49.8~GW facility-basis Western 2030 horizon under five PUE assumptions. The central IT-load bridge is 42.5~GW under row-level PUE assumptions. Comparisons against utility-contracted-MW or grid-interconnect datasets should use the facility figures; comparisons against chip-nameplate aggregates should use the IT-load equivalents.}
\end{table}

At facility baseline 49.8~GW, the IT-load-equivalent capacity falls in the range roughly $[38, 45]$~GW depending on PUE, with a row-level central bridge of 42.5~GW. The PUE range 1.10--1.35 captures where every currently-announced frontier-AI campus operates at scheduled commissioning.
...
By 2030 on an announced-horizon basis, the same operator set expands to approximately \textbf{49.8~GW facility} aggregate (raw; 42.5~GW IT-load bridge); to approximately \textbf{35.1~GW facility} probability-weighted; and to approximately \textbf{26.3~GW facility} under the conservative T1+T2+T3 case. The three largest operators (Microsoft, Amazon-including-Anthropic, OpenAI-Stargate) each individually carry more than 10~GW of announced buildout. Amazon/Anthropic's 12.9~GW is dominated by the T4 Anthropic expansion announcement (3.17~GW incremental at 72\% realization), OpenAI/Stargate's 10.6~GW is dominated by T4 Stargate 8 sites (at a blended 70\% realization because Stargate is capex-secured but grid-interconnection-constrained), and Microsoft's 5.3~GW sits largely at T2 (Fairwater construction visible) with realization probabilities near 85\%.
...
\textbf{Approximately 85\% of the 49.8~GW facility raw horizon is not yet operational; probability-weighted expected realization by 2030 is approximately 35.1~GW facility, of which roughly 78\% is not yet operational today.} The build-out is therefore a forward execution bet, not an in-place compute base. The principal risks are concentrated in four categories: grid interconnection, chip delivery, counterparty concentration, and the realization of outer-year stretch targets (particularly Meta Hyperion, Reliance Jamnagar, and the Anthropic--Amazon full 5~GW). Each is priced into the scenario matrix in \S\ref{subsec:stress-matrix}.

\subsection{Confidence decomposition of the 2030 horizon}

Of the 49.8~GW facility Western horizon, the six-tier evidence split is: \textbf{T1 Operational} 15\% (7.37~GW, 100\% realization), \textbf{T2 Under construction / physically evidenced} 25\% (12.41~GW, 88\% default), \textbf{T3 Firm commercial} 13\% (6.48~GW, 78\% default), \textbf{T4 Announced site-level plan} 33\% (16.48~GW, 58\% default), \textbf{T5 LOI / stretch target} 14\% (6.75~GW, 32\% default), and \textbf{T6 Analyst inference} 1\% (0.33~GW, 25\% default). Probability weighting across the six tiers yields approximately \textbf{35.1~GW facility} as the deterministic expected 2030 realization, materially below the 49.8~GW announced horizon and materially above the 26.3~GW conservative case (T1+T2+T3 only). The Monte Carlo median (\S\ref{subsec:monte-carlo}) at approximately 31.4~GW facility is the base realization case.
...
\addplot[fill=chartG,draw=white] coordinates {(12.41,horizon)};
...
\addplot[fill=accentOchre,draw=white] coordinates {(16.48,horizon)};
...
\end{axis}
\end{tikzpicture}
\caption{Distribution of the 49.8~GW facility Western horizon across six evidence tiers. Per-tier realization probability is the tier-default midpoint with per-row override possible. Probability-weighted sum across the six tiers = approximately \textbf{35.1~GW facility}; the next figure shows the same segments re-scaled by tier-default probability.\protect\footnotemark}
\end{figure}
...
\addplot[fill=chartG,draw=white] coordinates {(10.92,horizon)};
\addlegendentry{T2 $\times$ 0.88 = 10.92}
\addplot[fill=chartA,draw=white] coordinates {(5.05,horizon)};
\addlegendentry{T3 $\times$ 0.78 = 5.05}
\addplot[fill=accentOchre,draw=white] coordinates {(9.56,horizon)};
\addlegendentry{T4 $\times$ 0.58 = 9.56}
...
\end{axis}
\end{tikzpicture}
\caption{Same six tiers as the preceding figure, with each segment re-scaled by its tier-default realization probability. The expected 2030 Western realization is approximately \textbf{35.1~GW facility}, the right-hand edge of the stacked bar. The Monte Carlo median is approximately 31.4~GW facility after the two-sided scenario engine (\S\ref{subsec:monte-carlo}). The T1+T2+T3 conservative subset (7.37 + 10.92 + 5.05 = 23.3~GW expected, or 26.3~GW announced facility) sits as the conservative reading; the 53.1~GW facility arithmetic ceiling as the full-realization case. Basis is facility power; see \S\ref{subsec:power-units}.}
\end{figure}

\subsection{Scenario ranges: non-stretch 43.1 GW to full-realization 53.1 GW (facility)}

Stripping the stretch-target-only tier (T5) and compressing speculative completions yields a \textbf{raw non-stretch reading} of approximately \textbf{43.1~GW facility} (announced minus T5). Realising every stretch on disclosed timeline yields a \textbf{full-realization ceiling} of approximately \textbf{53.1~GW facility}. The deterministic tier-weighted base of \textbf{35.1~GW facility} sits between the two because tier-default probabilities discount each tier. The Monte Carlo median (\S\ref{subsec:monte-carlo}) falls lower still at approximately \textbf{31.4~GW facility}, reflecting per-tier variance, correlated downside states, demand/RPO stress, and upside cases.
...
The 35.1~GW facility tier-weighted horizon is a single point. The realization distribution combines tier-level uncertainty with the scenario architecture above. Each draw samples realization rates by tier, applies the systemic infrastructure state, samples conditional physical-capacity shocks, applies demand/RPO stress, and then applies the independent silicon, mix, and upside cases.
...
\bottomrule
\end{tabular}
\caption{Monte Carlo distribution of the 2030 Western realized horizon (facility basis), 10{,}000 draws. Seed 20260424, tier rollup T1=7.37, T2=12.41, T3=6.48, T4=16.48, T5=6.75, T6=0.33 facility. \textbf{The median (31.4~GW facility) sits below the 35.1~GW facility tier-weighted point} because downside mechanisms remain larger than the modeled upside states. This is a realization distribution, not an accounting horizon. IT-load bridge at blended PUE~1.20: approximately 27.4~GW IT (see \S\ref{subsec:power-units}).}
\end{table}

\textbf{Tail probabilities from the simulation (facility basis):}
\begin{itemize}\itemsep0pt
\item $P(\text{realized 2030 GW} < 26.3$ conservative case$) \approx 24.0\%$
\item $P(\text{realized 2030 GW} < 35.1$ tier-weighted point$) \approx 75.8\%$
\item $P(\text{realized 2030 GW} < 43.1$ raw non-stretch$) = 100\%$
\item $P(\text{realized 2030 GW} < 49.8$ announced horizon$) = 100\%$
\end{itemize}

We treat the Monte Carlo median of approximately \textbf{31.4~GW facility} as the base realization case, with a p10--p90 interdecile range of \textbf{[23.4, 37.2]~GW facility}. The announced 49.8~GW and raw non-stretch 43.1~GW horizons are accounting totals, not expected realized outcomes. For stress testing, p10 (approximately 23.4~GW facility) is the downside case.
...
\midrule
45\% & 34.3 & 30.7 & 22.6--35.1 \\
58\% base & 35.1 & 31.4 & 23.4--37.2 \\
70\% & 37.2 & 34.7 & 26.8--40.3 \\
\bottomrule
\end{tabular}
\caption{T4 realization-probability sensitivity, facility basis. T4 is the most judgment-sensitive tier. A 13-point reduction in the T4 default realization probability moves the deterministic horizon by approximately 2.1~GW before scenario states ($16.48 \times 0.13 \approx 2.14$). Monte Carlo rows use the same two-sided scenario architecture described above.}
\end{table}
...
\end{axis}
\end{tikzpicture}
\caption{Sovereign-AI pipeline, 2030 target GW. \textbf{Unit:} GW facility power at commissioning; IT-load bridge is reported separately from row-level PUE assumptions. \textbf{Rows:} Stargate UAE (1.40~GW), UAE G42--Khazna (0.26~GW), xAI--HUMAIN Saudi (0.50~GW), AMD--HUMAIN Saudi (0.50~GW), UK Culham AI Growth Zone (0.30~GW), and Reliance Jamnagar (1.00~GW). \textbf{Aggregate:} 3.96~GW facility announced (approximately 3.12~GW IT-load bridge), 1.83~GW facility probability-weighted. \textbf{Not included in Western 49.8~GW facility denominator.}\protect\footnotemark}
\end{figure}
...
Because a single gigawatt of capacity may be referenced across multiple announcements (e.g., Microsoft's Fairwater commitment and OpenAI's Stargate reference; Amazon's Project Rainier and Anthropic's Trainium commitments), capacity was consolidated at the site level. Where an announcement references an existing Epoch-tracked site, the announcement's reported capacity replaces the Epoch figure only if it exceeds the Epoch buildout; otherwise the Epoch figure is retained. Chip procurement commitments (OpenAI--NVIDIA 10~GW, OpenAI--AMD 6~GW, OpenAI--Broadcom 10~GW, Anthropic--Google 4.5~GW TPU) are recorded as chip-vendor commitments rather than physical-site commitments, since the chips deploy into Stargate, Azure, Fluidstack, or Google-owned shells already accounted for at the site level. The resulting unduplicated physical-site census is the 49.8 GW facility Western 2030 horizon referenced throughout this paper; the corresponding IT-load bridge is 42.5 GW under row-level PUE assumptions.
...
\noindent The full row-level audit ships as the companion file \texttt{row\_level\_audit.csv} alongside this paper: \texttt{row\_level\_audit.csv} contains 81 capacity atoms \texttimes{} 40 columns. Each row carries commitment ID, operator, tenant, evidence tier (T1--T6), realization probability, MW basis, PUE assumption, source URL, \texttt{source\_publication\_date}, \texttt{last\_verified\_date}, \texttt{freshness\_status}, \texttt{freshness\_waiver\_reason}, dedupe fields, and notes. Aggregates in the paper can be reproduced by filtering the companion audit file on the relevant inclusion and denominator fields.

\medskip
The summary tables below separate the Western denominator from the sovereign sidebar. Rows with \texttt{western\_denominator=false} are not part of the Western 2030 horizon even when they are large physical commitments. The overlay incremental total of approximately 18.4~GW facility, combined with Epoch's 31.45~GW physically-evidenced floor, yields the 49.8~GW facility Western 2030 horizon cited throughout the paper; the 3.96~GW sovereign sidebar is reported separately.
...
\midrule
\textbf{Sovereign sidebar total} & & & \textbf{3.96} & & & \textbf{No} \\
\bottomrule
\end{tabular}
