...
from Voltage~Park / Lightning and the xAI Colossus~2 residual). Announced commitments through 2030 imply a \textbf{raw horizon of approximately 51.4~GW facility
...
T6 at 25\%) the deterministic tier-weighted point estimate is approximately \textbf{36.2~GW
...
32.4~GW facility} with an \textbf{interdecile range of [24.4, 38.3]~GW} --- our
...
case} raw (Tiers~1+2+3 only) is approximately \textbf{26.5~GW facility}; the \textbf{full-realization
...
\textcolor{softGrey}{9.} & Sovereign AI & 27 \\
...
In Q2 2026, the Western-aligned tier-clean operational AI compute fleet totals approximately \textbf{7.56~GW} facility power (T1), with a further approximately \textbf{0.25~GW facility} T6-inferred from neocloud sources (Voltage Park / Lightning and the xAI Colossus~2 residual). The chip footprint is approximately \textbf{4.2~million H100-equivalent}.\footnote{Epoch AI, \emph{Frontier Data Centers} dataset, snapshot April 24, 2026, incorporating the 2026-04-22 changelog entry revising Stargate Abilene from a 600~MW April-1 milestone to ~200~MW actual operational. Epoch's ``Power (MW)'' field reports total facility power (GPUs + cooling + lighting + electrical losses); see \url{https://epoch.ai/data/data-centers-documentation}. H100-equivalent (H100e) normalizes GPU throughput by 8-bit operations per second relative to the NVIDIA H100 baseline.} Announced commitments through 2030 across the same operators, including hyperscalers, frontier labs, and the independent neoclouds, imply a raw \textbf{51.4~GW facility} horizon across the Western-aligned operator set (\textbf{43.9~GW} IT-load bridge), with a further approximately \textbf{2.56~GW facility} in the sovereign-AI sidebar. Under tier-default probability weighting, the expected 2030 realization is approximately \textbf{36.2~GW facility}; the T1+T2+T3 conservative case is approximately \textbf{26.5~GW facility}; and the full-realization ceiling is approximately \textbf{54.7~GW facility}.

\textbf{The central fact} is that approximately 85\% of the 51.4~GW facility raw 2030 horizon is not yet operational. On a probability-weighted basis, the expected realization of approximately 36.2~GW facility leaves approximately 78\% still to build, and the Monte Carlo median of approximately 32.4~GW facility (\S\ref{subsec:monte-carlo}) implies that even in the base realization case approximately 75\% of the 2030 horizon remains to be built. The AI compute stack of the late 2020s is overwhelmingly a forward commitment, not an in-place fleet.
...
\textbf{Where this section ends and the next begins.} Section~\ref{sec:scale} states the headline aggregates --- 7.56~GW tier-clean operational capacity, 51.4~GW raw 2030 horizon, 32.4~GW Monte~Carlo median, approximately \$1.9~trillion capital envelope, 78.2~million H100-equivalents. These are scale numbers, multiplications. Before scaling, the multiplicand. The next section decomposes one AI facility gigawatt into its physical bill of materials so every aggregate downstream resolves to a unit that has been priced, assembled, and reconciled to the external literature. We then return in Section~\ref{sec:scale-body} to the power-unit conventions, the compute-versus-physical-capacity bridge, and the operator concentration on which the headline numbers rest.
...
Multiplying one facility GW by the announced horizon resolves the headline numbers. The Western 2026--2030 capital envelope at the raw 51.4~GW announced horizon is approximately \textbf{\$1.9~trillion central, with a defensible range of \$1.5--2.4~trillion} depending on operator mix, BTM-generation share, and silicon-vintage assumptions.
...
\midrule
Monte~Carlo p10 (\S\ref{subsec:monte-carlo}) & 24.4 & \$0.73--1.15T \\
Scenario-adjusted median (Monte~Carlo p50) & 32.4 & \$0.97--1.52T \\
Deterministic tier-weighted point & 36.2 & \$1.08--1.70T \\
Monte~Carlo p90 & 38.3 & \$1.15--1.80T \\
Raw non-stretch horizon (announced minus T5) & 44.7 & \$1.34--2.10T \\
\textbf{Raw announced horizon} & \textbf{51.4} & \textbf{\$1.54--2.42T (central \$1.9T)} \\
Full-realization ceiling & 54.7 & \$1.64--2.57T \\
\bottomrule
\end{tabular}
...
1.60 (legacy colocation) & 32.4 & $-$37.5\% \\
\bottomrule
\end{tabular}
...
By 2030 on an announced-horizon basis, the same operator set expands to approximately \textbf{51.4~GW facility} aggregate (raw; 43.9~GW IT-load bridge); to approximately \textbf{36.2~GW facility} probability-weighted; and to approximately \textbf{26.5~GW facility} under the conservative T1+T2+T3 case. The three largest operators (Microsoft, Amazon-including-Anthropic, OpenAI-Stargate) each individually carry more than 10~GW of announced buildout. Amazon/Anthropic's 12.9~GW is dominated by the T4 Anthropic expansion announcement (3.17~GW incremental at 72\% realization), OpenAI/Stargate's 10.6~GW is dominated by T4 Stargate 8 sites (at a blended 70\% realization because Stargate is capex-secured but grid-interconnection-constrained), and Microsoft's 5.3~GW sits largely at T2 (Fairwater construction visible) with realization probabilities near 85\%.
...
The \$1.9~T capital envelope has two equally valid aggregate views. The capex bridge below shows \emph{what the money physically buys}; the financing bridge shows \emph{whose balance sheet the money flows off}. The two views reconcile at the \$1.9~T central but use different convention bounds (arithmetic layer-extreme for the capex bridge \$1.4--2.7~T, credible-source range for the financing bridge \$1.65--2.25~T; both sit inside the headline defensible band \$1.6--2.4~T). The \$550B of contracted remaining performance obligations (RPO) is \emph{neither} --- it is the revenue underwriting that \emph{enables} the debt-financing channel (Oracle's \$300B from OpenAI, Anthropic's \$100B+ from Amazon, Nebius's \$27B from Meta, etc.\ are lab \emph{revenue} commitments, not capex channels; rev-2 mis-depicted RPO as a stand-alone funding source and is corrected here).
...
\addplot[fill=chartA,draw=white] coordinates {(6.48,horizon)};
\addlegendentry{T3 Firm commercial (prob=0.78)}
\addplot[fill=accentOchre,draw=white] coordinates {(17.88,horizon)};
...
\addplot[fill=chartA,draw=white] coordinates {(5.05,horizon)};
\addlegendentry{T3 $\times$ 0.78 = 5.05}
\addplot[fill=accentOchre,draw=white] coordinates {(10.37,horizon)};
\addlegendentry{T4 $\times$ 0.58 = 10.37}
...
\end{axis}
\end{tikzpicture}
\caption{Same six tiers as the preceding figure, with each segment re-scaled by its tier-default realization probability. The expected 2030 Western realization is approximately \textbf{36.2~GW facility}, the right-hand edge of the stacked bar. The Monte Carlo median is approximately 32.4~GW facility after the two-sided scenario engine (\S\ref{subsec:monte-carlo}). The T1+T2+T3 conservative subset (7.56 + 10.93 + 5.05 = 23.5~GW expected, or 26.5~GW announced facility) sits as the conservative reading; the 54.7~GW facility arithmetic ceiling as the full-realization case. Basis is facility power; see \S\ref{subsec:power-units}.}
\end{figure}

\subsection{Scenario ranges: non-stretch 44.7 GW to full-realization 54.7 GW (facility)}

Stripping the stretch-target-only tier (T5) and compressing speculative completions yields a \textbf{raw non-stretch reading} of approximately \textbf{44.7~GW facility} (announced minus T5). Realising every stretch on disclosed timeline yields a \textbf{full-realization ceiling} of approximately \textbf{54.7~GW facility}. The deterministic tier-weighted base of \textbf{36.2~GW facility} sits between the two because tier-default probabilities discount each tier. The Monte Carlo median (\S\ref{subsec:monte-carlo}) falls lower still at approximately \textbf{32.4~GW facility}, reflecting per-tier variance, correlated downside states, demand/RPO stress, and upside cases.
...
Neocloud financing stress & $-2.0$ & $-74$ & CoreWeave, Nebius, Applied Digital, Crusoe; conditional probability 11.8\% if systemic and RPO stress do not fire, 60\% if RPO stress fires & 25\% target \\
...
The 36.2~GW facility tier-weighted horizon is a single point. The realization distribution combines tier-level uncertainty with the scenario architecture above. Each draw samples realization rates by tier, applies the systemic infrastructure state, samples conditional physical-capacity shocks, applies demand/RPO stress, and then applies the independent silicon, mix, and upside cases.
...
58\% base & 36.2 & 32.4 & 24.4--38.3 \\
70\% & 38.3 & 34.7 & 26.8--40.3 \\
\bottomrule
\end{tabular}
\caption{T4 realization-probability sensitivity, facility basis. T4 is the most judgment-sensitive tier. A 13-point reduction in the T4 default realization probability moves the deterministic horizon by approximately 2.3~GW before scenario states ($16.41 \times 0.13 \approx 2.13$). Monte Carlo rows use the same two-sided scenario architecture described above.}
\end{table}
